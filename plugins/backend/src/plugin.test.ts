...
import { datacontract } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [datacontract],
    });

    await request(server).get('/api/backstage-plugin-datacontract-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/backstage-plugin-datacontract-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/backstage-plugin-datacontract-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/backstage-plugin-datacontract-backend/todos/${createdTodoItem.id}`)
