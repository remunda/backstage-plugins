...
export const DataContractPage: React.FC<DataContractPageProps> = ({ definition }) => {
      ...
      <Page themeId="tool">
        ...
        <Content>
          <Alert severity="error">
            ...
            Failed to parse the data contract definition: {error instanceof Error ? error.message : 'Unknown error'}
          </Alert>
        </Content>
      </Page>
    );
          ...
          <Grid item xs={12}>
            ...
            <Typography variant="body1">
              ...
              {dataContract.info.contact.url && (
                <>
                  {' - '}
                  <a href={dataContract.info.contact.url} target="_blank" rel="noopener noreferrer">
                    {dataContract.info.contact.url}
                  </a>
                </>
              )}
  ...
  const renderServersCard = () => {
    if (!dataContract.servers || Object.keys(dataContract.servers).length === 0) {
      return null;
    }
          ...
          <Box key={serverName} className={classes.section}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Typography variant="body1">{server.format || 'N/A'}</Typography>
              </Grid>
              <Grid item xs={12}>
                <Typography variant="subtitle2" color="textSecondary">
                  Location
                </Typography>
                <Typography variant="body1" style={{ fontFamily: 'monospace', wordBreak: 'break-all' }}>
                  {server.location || 'N/A'}
                </Typography>
              </Grid>
            </Grid>
          ...
          <Box key={modelName} className={classes.section}>
            ...
            {model.fields && (
              <Table
                ...
                data={Object.entries(model.fields).map(([fieldName, field]) => ({
                  name: fieldName,
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    field: 'description',
                    render: (rowData) => {
                      const data = rowData as { description: string; example: string; constraints: string[] };
                      return (
                        <Box>
            ...
            {model.primaryKey && (
              <Box marginTop={2}>
                <Typography variant="subtitle2" color="textSecondary">
                  Primary Key: <span style={{ fontFamily: 'monospace' }}>{model.primaryKey}</span>
                </Typography>
              </Box>
            )}
            <Divider style={{ margin: '16px 0' }} />
